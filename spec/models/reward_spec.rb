...
RSpec.describe Reward, type: :model do
  before(:all) do
    ...
    @manager.rewards << @reward
    @manager.rewards << @reward_2
  end
  ...
  context "Associations" do
    it "has many-to-many relationship with PointRewardManager" do
      expect(@reward.point_reward_managers.count).to eq 1
      expect(@reward_2.point_reward_managers.count).to eq 1
      expect(@manager.rewards).to eq [@reward, @reward_2]
    end
  end
end
